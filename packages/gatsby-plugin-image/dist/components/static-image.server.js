...
function _getStaticImage(GatsbyImage) {
    return function StaticImage(_a) {
        ...
        width = _a.width, maxWidth = _a.maxWidth, height = _a.height, maxHeight = _a.maxHeight, tracedSVGOptions = _a.tracedSVGOptions, placeholder = _a.placeholder, formats = _a.formats, quality = _a.quality, transformOptions = _a.transformOptions, jpgOptions = _a.jpgOptions, pngOptions = _a.pngOptions, webpOptions = _a.webpOptions, avifOptions = _a.avifOptions, blurredOptions = _a.blurredOptions, 
        /* eslint-enable @typescript-eslint/no-unused-vars */
        props = __rest(_a, ["src", "__imageData", "__error", "width", "maxWidth", "height", "maxHeight", "tracedSVGOptions", "placeholder", "formats", "quality", "transformOptions", "jpgOptions", "pngOptions", "webpOptions", "avifOptions", "blurredOptions"]);
        if (__error) {
...
var checkDimensionProps = function (props, propName) {
    ...
    if (props.layout !== "fixed" &&
        ...
        props[propName]) {
        return new Error("\"" + propName + "\" " + props[propName] + " may not be passed when layout is \"" + (props.layout || "constrained") + "\"");
    }
    else {
        if (props.layout === "fixed" &&
            (propName === "maxWidth" || propName === "maxHeight") &&
            props[propName]) {
            return new Error("\"" + propName + "\" may not be passed when layout is \"" + props.layout + "\"");
        }
    }
    return prop_types_1["default"].number.apply(prop_types_1["default"], __spread([props, propName], rest));
};
var validLayouts = new Set(["fixed", "fluid", "constrained"]);
exports.propTypes = {
    ...
    layout: function (props) {
        ...
        return new Error("Invalid value " + props.layout + "\" provided for prop \"layout\". Defaulting to \"fixed\". Valid values are \"fixed\", \"fluid\" or \"constrained\"");
    }
};
